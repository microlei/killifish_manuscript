...
p<-ggplot(swan_wild %>% filter(attack=="degree"), aes(x=fraction, y=value, color=fishType)) + geom_point(size=4) + scale_color_manual(values = c(cols.fishType[1], cols.fishType[3])) + labs(x="Fraction of nodes removed", y=expression("Loss of connectivity \U2192")) + theme_light()
...
ggsave(p, filename = here("figures/netswan.png"), width=4, height=4, units="in")
